...
- **Event immutability:** once persisted, a registration's `event` is **locked** to the event it was created for (FR-14/FR-37/TC-045) — reparenting would strand the destination event's ID sequence. Enforced by `clean()` (friendly admin error) + a `save()` backstop covering every code path; the admin renders `event` read-only on change forms (editable on add). Add a new row to put an owner in another event.
